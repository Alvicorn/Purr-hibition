...
void AudioMixer_init(void)
{
	...
	// Open the PCM output
	int err = snd_pcm_open(&handle, "plughw:Audio,0", SND_PCM_STREAM_PLAYBACK, 0);
	if (err < 0) {
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	 ...
	 */
	 ...
	 pthread_mutex_unlock(&audioMutex);
	 printf("ERROR: No free sound bite slots available.\n");
    ...
    for (int i = 0; i < MAX_SOUND_BITES; i++) {
        if (soundBites[i].pSound != NULL) {
            for (int j = 0; j < size; j++) {
                ...
                int sample = buff[j] + soundBites[i].pSound->pData[soundBites[i].location];
                if (sample > SHRT_MAX) {
                    sample = SHRT_MAX;
